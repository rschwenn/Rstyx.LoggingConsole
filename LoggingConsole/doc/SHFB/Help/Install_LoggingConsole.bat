...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale en-us /uninstall /silent /vendor "Rstyx" /productName "Frameworks and Applications" /mediaBookList "LoggingConsole" > NUL
...
REM build time.
IF EXIST "LoggingConsole.msha" COPY /Y "LoggingConsole.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion 2.0 /catalogName VisualStudio12 /locale en-us /wait 0 /operation uninstall /vendor "Rstyx" /productName "Frameworks and Applications" /bookList "LoggingConsole" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion 2.0 /catalogName VisualStudio12 /locale en-us /wait 0 /operation install /sourceUri "%CD%\LoggingConsole.msha"
